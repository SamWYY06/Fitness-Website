...
<div class = "container">
    ...
    <label for="paymentMethod">Select a payment method:</label>
        ...
        <div id="Touch n Go" class="tab-content" style="display: none;">
            ...
            <img src="tng.png" width="400" height="auto">
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($formType == "consultationForm") {
        // Existing consultation form handling code remains the same
        ...
        // Insert payment data
        $paymentSql = "INSERT INTO payments (user_id, paymentFor, paymentMethod, paymentAmount, paymentDate) 
                      VALUES (?, ?, ?, ?, ?)";
        $paymentStmt = mysqli_prepare($conn, $paymentSql);
        mysqli_stmt_bind_param($paymentStmt, "issss", $user_id, $paymentFor, $paymentMethodInput, $paymentAmount, $paymentDate);
        
        if (mysqli_stmt_execute($paymentStmt)) {
            $payment_id = mysqli_insert_id($conn); // Get the ID of the inserted payment

            // Handle different payment scenarios
            if ($paymentFor == "Consultation" && isset($_SESSION['preferred_date'])) {
                // Get a random consultant name from the Consultant table
                $consultantSql = "SELECT ConsultantName FROM Consultant ORDER BY RAND() LIMIT 1";
                $consultantResult = mysqli_query($conn, $consultantSql);
                $consultantRow = mysqli_fetch_assoc($consultantResult);
                $randomConsultantName = $consultantRow['ConsultantName'];

                $consultationSql = "INSERT INTO consultations (user_id, preferredDate, preferredTime, consultantName, notes, payment_id)
                                    VALUES (?, ?, ?, ?, ?, ?)";
                $consultationStmt = mysqli_prepare($conn, $consultationSql);
                mysqli_stmt_bind_param($consultationStmt, "isssss", $user_id, 
                    $_SESSION['preferred_date'],
                    $_SESSION['preferred_time'],
                    $randomConsultantName,
                    $_SESSION['notes'], 
                    $payment_id
                );
                ...
                if (mysqli_stmt_execute($consultationStmt)) {
                    ...
                    // If consultation insert fails, roll back the payment
                    $rollbackSql = "DELETE FROM payments WHERE paymentID = ?";
                    $rollbackStmt = mysqli_prepare($conn, $rollbackSql);
                    mysqli_stmt_bind_param($rollbackStmt, "i", $payment_id);
            ...
            elseif (in_array($paymentFor, array('Gold Membership', 'Silver Membership', 'Bronze Membership'))) {
                // Extract membership type from paymentFor
                $membership_type = str_replace(' Membership', '', $paymentFor);
                ...
                $end_date = date('Y-m-d', strtotime("+{$duration_months[$membership_type]} months"));
                
                // First, check if user has any active membership
                $checkActiveSql = "UPDATE memberships 
                                 SET status = 'expired' 
                                 WHERE user_id = ? 
                                 AND status = 'active' 
                                 AND end_date >= CURDATE()";
                $checkActiveStmt = mysqli_prepare($conn, $checkActiveSql);
                mysqli_stmt_bind_param($checkActiveStmt, "i", $user_id);
                mysqli_stmt_execute($checkActiveStmt);
                
                // Insert new membership
                $membershipSql = "INSERT INTO memberships 
                                (user_id, membership_type, start_date, end_date, payment_id, status) 
                                VALUES (?, ?, ?, ?, ?, 'active')";
                $membershipStmt = mysqli_prepare($conn, $membershipSql);
                mysqli_stmt_bind_param($membershipStmt, "isssi", 
                    $user_id, 
                    $membership_type, 
                    $start_date, 
                    $end_date, 
                ...
                if (mysqli_stmt_execute($membershipStmt)) {
                    echo "<script>alert('Payment successful! Your " . $membership_type . " membership is now active until " . date('d-m-Y', strtotime($end_date)) . "'); window.location.href='home.php';</script>";
                } else {
                    echo "Error processing membership: " . mysqli_error($conn);
                }
            }
            else {
